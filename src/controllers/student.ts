...
export const getStudentContestStats = async (req: Request, res: Response) => {
  try {
    ...
    const firstRating =
      contests.length > 0
        ? contests[0].oldRating || contests[0].newRating || 0
        : 0;
    const lastRating =
      contests.length > 0 ? contests[contests.length - 1].newRating || 0 : 0;
...
export const getStudentProblemStats = async (req: Request, res: Response) => {
  try {
    ...
    const solvedProblems = await submissions
      .find({
        studentId: id,
        verdict: "OK",
    ...
    const uniqueProblems = solvedProblems.reduce(
      (acc, submission) => {
    ...
    const problemsWithRating = uniqueSolvedProblems.filter(
      (p) => p.problemRating && p.problemRating > 0
    );
    const ratingsSum = problemsWithRating.reduce(
      (sum, problem) => sum + problem.problemRating,
      0
    );
    const averageRating =
      problemsWithRating.length > 0
        ? Math.round(ratingsSum / problemsWithRating.length)
        : 0;
    ...
    const mostDifficult = problemsWithRating.reduce((max, problem) => {
      return (problem.problemRating || 0) > (max?.problemRating || 0)
        ? problem
        : max;
    }, null as any);
    ...
    problemsWithRating.forEach((problem) => {
      ...
      else if (rating >= 1000 && rating <= 1199)
        ratingDistribution["1000-1199"]++;
      else if (rating >= 1200 && rating <= 1399)
        ratingDistribution["1200-1399"]++;
      else if (rating >= 1400 && rating <= 1599)
        ratingDistribution["1400-1599"]++;
      else if (rating >= 1600 && rating <= 1799)
        ratingDistribution["1600-1799"]++;
      else if (rating >= 1800 && rating <= 1999)
        ratingDistribution["1800-1999"]++;
...
export const getStudentRecentProblems = async (req: Request, res: Response) => {
  try {
    ...
    const recentProblems = await submissions
      .find({
        studentId: id,
        verdict: "OK",
        submissionTime: { $gte: fromDate },
      ...
      .limit(limit)
      .select(
        "problemId problemName problemRating problemTags submissionTime language timeMs memoryBytes"
      );
...
export const checkSyncStatus = async (
  ...
  const result = await syncStudentSubmissions(req.params.id);

  if (!result.success) {
    if (result.dataOutdated) {
  ...
  res.json({
    ...
    revalidation: result.dataOutdated,
  });
};

export const checkContestSyncStatus = async (
  req: Request,
  res: Response
): Promise<void | any> => {
  const result = await syncStudentContests(req.params.id);
  ...
  res.json({
    message: result.message,
    newContests: result.newContests,
  });
};
